...
Page({
    ...
    //选择id
    query.select('.goods-area').boundingClientRect(function (rect) {
      that.setData({
      s_height: rect.height//可滑动区域高度
      })
    }).exec();
...
console.log(options)
    if (options.storeuid) {//扫码进来
      ...
      if (store){
        ...
        if(app.globalData.scene == "1011"){ //根据场景值判断是扫公众码进来还是没有门店的情况
          wx.redirectTo({ 
  ...
  scrollmove: function (self,scrollTop) {
    var scrollArr = that.data.right_height_arr;
    ...
    app.req({
      ...
      success: function (data) {
        if (data.items && data.items.length > 0) {
          ...
          let showType = (eatingway.indexOf('1') != -1 && eatingway.indexOf('0') != -1)?true: false;
          if(!showType){
          ...
          app.open.tips(that, {
            ...
            yesCallback: function () {
              wx.navigateTo({
                url: '/pages/home/selectStore/selectStore',
              })
            }
          })
        }
          ...
          if (data.items.length > 0) {
            ...
            for (var i = 0; i < data.items.length; i++) {
              for (var j = 0; j < data.items[i].proditems.length; j++) {
                ...
                if ((good.exunititems && good.exunititems.length > 0) || (good.flaclassitems && good.flaclassitems.length > 0)) {
          ...
          if (val.prodclassuid == item.prodclassuid) {
            item.proditems.forEach(function (good, j) {
              if (good.produid == val.produid || good.produid == val.oproduid) {
                ...
                if (val.attributes == "" || val.attributes){
  ...
  gotoSettlement: function (event) {
    ...
    if (selectData.length > 0) {
      ...
      if (isAuthor) {//已经授权
        wx.navigateTo({
          url: '../placeOrder/placeOrder?eatType=' + that.data.eatType
        })
      }else{
        wx.navigateTo({
  ...
  searchGood:function(){
    wx.navigateTo({
      url: '../searchGood/searchGood?storeuid='+that.data.store.storeuid,
    })
  }
